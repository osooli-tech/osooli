    ...
    <header class="fixed top-0 end-0 start-0 h-16 z-30
        ...
        @can('parcels.view_map')
            {{-- The dashboard is the map: it hosts #sakuki-map, and there is no
                 separate map route — only the GeoJSON feed it reads. The
                 fragment scrolls down to it; without one, a click while
                 already on the dashboard only reloaded it at the top. --}}
