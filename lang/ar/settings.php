<?php

declare(strict_types=1);

return [
    'title' => 'إدارة الأدوار',
    'add_role' => 'إضافة دور',
    'role_name' => 'اسم الدور',
    'permissions_count' => 'صلاحية|صلاحيات',
    'edit_permissions' => 'تعديل الصلاحيات',
    'delete_role' => 'حذف',
    'save' => 'حفظ',
    'cancel' => 'إلغاء',
    'create_title' => 'إضافة دور جديد',
    'edit_title' => 'صلاحيات دور: :role',
    'delete_confirm' => 'هل أنت متأكد من حذف الدور ":role"؟',
    'confirm_delete_btn' => 'نعم، احذف',
    'role_created' => 'تم إنشاء الدور',
    'role_deleted' => 'تم حذف الدور',
    'permissions_saved' => 'تم حفظ الصلاحيات',
    'role_has_users' => 'لا يمكن حذف الدور — يوجد مستخدمون مرتبطون به',
    'no_permissions' => 'لا توجد صلاحيات',
    'selected' => 'محددة',

    'groups' => [
        'parcels' => 'القطع والخريطة',
        'deeds' => 'الصكوك والملكية',
        'survey' => 'القرارات المساحية',
        'reference' => 'البيانات المرجعية',
        'documents' => 'المستندات',
        'archive' => 'الأرشيف',
        'exports' => 'التصدير',
        'requests' => 'الطلبات',
        'owners' => 'المُلّاك',
        'users' => 'المستخدمون',
        'admin' => 'الإدارة',
    ],

    'perm' => [
        'parcels_view' => 'عرض القطع',
        'parcels_view_map' => 'عرض الخريطة',
        'parcels_create' => 'إضافة قطعة',
        'parcels_edit' => 'تعديل القطع والحدود',
        'parcels_edit_geometry' => 'تعديل مضلعات القطع ورسمها',
        'parcels_archive' => 'أرشفة القطع',
        'deeds_create' => 'إضافة صك',
        'deeds_edit' => 'تعديل الصكوك',
        'deeds_archive' => 'أرشفة الصكوك',
        'ownership_manage' => 'إدارة المُلّاك ونسب الملكية',
        'survey_decisions_edit' => 'تعديل القرارات المساحية',
        'reference_view' => 'عرض البيانات المرجعية',
        'reference_create' => 'إضافة بيانات مرجعية',
        'reference_edit' => 'تعديل البيانات المرجعية',
        'reference_delete' => 'حذف بيانات مرجعية',
        'documents_download' => 'تنزيل المستندات',
        'documents_upload' => 'رفع المستندات',
        'documents_review' => 'مراجعة المستندات واعتمادها',
        'archive_view' => 'عرض الأرشيف',
        'archive_restore' => 'استرجاع المؤرشف',
        'exports_create' => 'تصدير البيانات',
        'exports_bulk' => 'التصدير الشامل للصكوك (GeoJSON)',
        'owners_create' => 'إضافة مالك',
        'owners_edit' => 'تعديل بيانات المُلّاك',
        'presentation_requests_view' => 'عرض طلبات العرض التقديمي',
        'modification_requests_view' => 'عرض طلبات التعديل',
        'modification_requests_manage' => 'إدارة طلبات التعديل',
        'users_view' => 'عرض المستخدمين',
        'users_create' => 'إضافة مستخدم',
        'users_edit' => 'تعديل مستخدم',
        'users_delete' => 'حذف مستخدم',
        'roles_manage' => 'إدارة الأدوار',
        'audit_logs_view' => 'عرض سجل العمليات',
        'sync_view' => 'عرض التزامن',
        'database_manage' => 'إدارة قواعد البيانات والمزامنة',
    ],
];
