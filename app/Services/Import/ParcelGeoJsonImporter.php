 ...
 * artisan command and the dashboard import run exactly the same code, on
 * whichever database is primary (PostgreSQL or MariaDB).
...
final class ParcelGeoJsonImporter implements Importer
{
    ...
    public function importFeatures(array $features): ImportResult
    {
        ...
        foreach ($groups as $group) {
            ...
            // later blank-Geo_ID feature then matches by geo_id, collapsing
            // unrelated parcels into one shared row. Skip instead — the
            // preview already promises this.
            if ($geoId === '') {
                $skipped += count($group);
    ...
    private function importGroup(array $group, ?int $cityId, int $officeId, array &$stats): void
    {
         ...
         * made, and an import should not quietly undo it. Restoring is
        ...
        // Deed — where() with a null deed_no compiles to IS NULL, so a deed
        // without a number still matches itself on the next run.
